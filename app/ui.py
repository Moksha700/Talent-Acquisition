...
from app.utils.email import send_automatic_emails
...
def ui() -> None:
    ...
    with setup_tab:
        ...
        email_configured = all(os.getenv(var) for var in ['EMAIL_USER', 'EMAIL_PASS', 'FROM_EMAIL'])
        
        if not email_configured:
            st.info("📧 **Email Setup**: To enable real email sending, set EMAIL_USER, EMAIL_PASS, and FROM_EMAIL environment variables. Currently running in demo mode.")
        else:
            st.success("✅ **Email credentials configured** - Real emails will be sent")
        
        email_config_col1, email_config_col2 = st.columns(2)
        with email_config_col1:
            interview_threshold = st.slider(
                "Interview Threshold", 
                min_value=0.0, 
                max_value=1.0, 
                value=CONFIG.interview_threshold, 
                step=0.05,
                help="Candidates with fit score above this threshold will receive interview emails"
            )
        with email_config_col2:
            rejection_threshold = st.slider(
                ...
                min_value=0.0, 
                max_value=1.0, 
                value=CONFIG.rejection_threshold, 
                step=0.05,
                help="Candidates with fit score below this threshold will receive rejection emails"
            )
        
        # Update config with user settings
        CONFIG.interview_threshold = interview_threshold
        CONFIG.rejection_threshold = rejection_threshold
        
        st.caption(f"📧 Email Settings: Interview emails for scores ≥ {interview_threshold:.2f}, Rejection emails for scores < {rejection_threshold:.2f}")
        ...
        if not cand_ids:
            ...
            st.markdown(f"**Found {len(candidates)} candidates** - All candidates will be automatically evaluated when you click 'Compute Fit & Generate Report'")
            # Display candidate list for reference
            with st.expander("View Available Candidates", expanded=False):
                for i, candidate in enumerate(candidates, 1):
                    st.write(f"{i}. {candidate.id}")
        
        # Use all candidates instead of selected ones
        chosen = candidates
        ...
        with col_a:
            ...
            if not selected_role:
                ...
                if auto_title:
                    ...
                    st.session_state["expanded_role"] = selected_role
            disabled = not (selected_role and len(chosen) > 0)
            if st.button("Compute Fit & Generate Report", help="Compute fit for all candidates and navigate to dashboard", key="analyze", use_container_width=True, disabled=disabled):
                orch = Orchestrator()
                ...
                email_results = send_automatic_emails(
                    candidates=chosen,
                    matches=[m.__dict__ for m in matches],
                    role_title=selected_role.title,
                    department=selected_role.department
                )
                st.session_state["email_results"] = email_results
                
                # Count email results
                interview_emails = len([r for r in email_results if r.email_type == "interview" and r.success])
                rejection_emails = len([r for r in email_results if r.email_type == "rejection" and r.success])
                failed_emails = len([r for r in email_results if not r.success])
                email_configured = all(os.getenv(var) for var in ['EMAIL_USER', 'EMAIL_PASS', 'FROM_EMAIL'])
                
                st.success(f"Analysis complete for {len(chosen)} candidates. Displaying results…")
                
                if email_configured:
                    if interview_emails > 0 or rejection_emails > 0:
                        st.success(f"📧 **REAL EMAILS SENT**: {interview_emails} interview invitations, {rejection_emails} rejection notices.")
                    else:
                        st.info("📧 No automatic emails sent (no candidates met threshold criteria).")
                else:
                    st.info("📧 **DEMO MODE**: Email functionality is working. Set EMAIL_USER, EMAIL_PASS, and FROM_EMAIL environment variables to enable real email sending.")
    ...
    with results_tab:
        ...
        if email_results:
            ...
            # Email statistics
            interview_sent = len([r for r in email_results if r.email_type == "interview" and r.success])
            rejection_sent = len([r for r in email_results if r.email_type == "rejection" and r.success])
            failed_emails = len([r for r in email_results if not r.success])
            no_email = len([r for r in email_results if r.email_type == "none"])
            
            col1, col2, col3, col4 = st.columns(4)
            with col1:
                st.metric("Interview Emails Sent", interview_sent)
            with col2:
                st.metric("Rejection Emails Sent", rejection_sent)
            with col3:
                st.metric("Failed Emails", failed_emails)
            with col4:
                st.metric("No Email Required", no_email)
            
            # Detailed email results
            with st.expander("View Detailed Email Results", expanded=False):
                for result in email_results:
                    if result.email_type != "none":
                        status_icon = "✅" if result.success else "❌"
                        st.write(f"{status_icon} **{result.candidate_id}** ({result.email_address}) - {result.email_type.title()}")
